...
class InitDialogWebSocketSessionUseCase @Inject constructor(private val repository: Repository) {
    suspend operator fun invoke(dialogId: Int) = withContext(Dispatchers.IO) {
        kotlin.runCatching { repository.initDialogWebSocketSession(dialogId) }
    }
}